...
public class AIAnalysisClient {
    ...
    public AIAnalysisResponse analyze(AIAnalysisRequest request) {
        ...
        try {
            ...
            log.info("AI service responded successfully. damagedPartsCount: {}", response.damagedParts().size());
            return response;

        } catch (RestClientResponseException ex) {
            throw new AIServiceException(
                    ErrorCode.AI_SERVICE_ERROR,
                    "AI service returned error. status: " + ex.getStatusCode() + " | body: " + ex.getResponseBodyAsString(),
                    ex);
        } catch (ResourceAccessException ex) {
            throw new AIServiceException(
                    ErrorCode.AI_SERVICE_ERROR,
                    "AI service is unreachable or timed out",
                    ex);
        }
