...
public class RefreshTokenServiceImpl implements RefreshTokenService {
     ...
     * @param familyId  pass an existing familyId to continue a rotation chain,
     *                  or pass null to start a fresh chain (new login)
     ...
     * 2. If REVOKED or not found → reject (possible replay after family kill).
     * 3. If USED → reuse detected → revoke the entire family → reject.
     * 4. If ACTIVE and not expired → mark as USED → issue new token in same family.
     */
    @Transactional
